...
export class CanvasUIMark {
  ...
  focusPrevious() {
    if (this.controls.length === 0) return;
    this.focusIndex =
      (this.focusIndex - 1 + this.controls.length) % this.controls.length;
  }
...
export class Toggle extends Control {
  ...
  isOverInteractiveArea(x, y) {
    ...
    return (
      x >= switchX &&
      x <= switchX + switchWidth &&
      y >= switchY &&
      y <= switchY + switchHeight
    );
  }
  ...
  draw(ctx, isFocused) {
    ...
    const switchX =
      this.x + this.width - switchWidth - this.options.padding - rightPadding;
    const switchY = this.y + (this.height - switchHeight) / 2;
    const switchRadius =
      this.options.borderRadius > 0
        ? Math.min(switchHeight / 2, this.options.borderRadius)
        : switchHeight / 2;

    // Switch background - use controlColor when on, backgroundColor when off
    ctx.fillStyle = this.value ? this.options.controlColor : "#333333"; // dark gray when off
    DrawRoundedRect(
      ctx,
      switchX,
      switchY,
    ...
    const knobX = this.value
      ? switchX + switchWidth - knobSize - knobPadding
      : switchX + knobPadding;
    const knobY = switchY + (switchHeight - knobSize) / 2;
    const knobRadius =
      this.options.borderRadius > 0
        ? Math.min(knobSize / 2, this.options.borderRadius)
        : knobSize / 2;
  ...
  isOverInteractiveArea(x, y) {
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      const distance = Math.sqrt(
        Math.pow(x - radioX, 2) + Math.pow(y - radioY, 2)
      );
...
export class Carousel extends Control {
  ...
  isOverInteractiveArea(x, y) {
    ...
    if (this.orientation === "horizontal") {
      return (
        this.containsPointInBounds(x, y, arrows.left) ||
        this.containsPointInBounds(x, y, arrows.right)
      );
    } else {
      return (
        this.containsPointInBounds(x, y, arrows.up) ||
  ...
  draw(ctx, isFocused) {
    ...
    if (this.orientation === "horizontal") {
      ...
      // Draw right arrow
      const rightArrowX =
        this.x + this.width - this.options.padding - this.arrowSize;
      ...
      const downArrowY =
        this.y + this.height - this.options.padding - this.arrowSize;
...
export class Slider extends Control {
  ...
  isOverInteractiveArea(x, y) {
    ...
    if (
      x >= knobX - knobMargin &&
      x <= knobX + knobSize + knobMargin &&
      y >= knobY - knobMargin &&
      y <= knobY + knobSize + knobMargin
    ) {
      return true;
    }
  ...
  draw(ctx, isFocused) {
    ...
    const trackRadius =
      this.options.borderRadius > 0
        ? Math.min(trackHeight / 2, this.options.borderRadius / 2)
        : trackHeight / 2;
    ...
    ctx.fillText(
      this.value.toString(),
      this.x + this.width - this.options.padding,
  ...
  constructor(manager, title, message, buttons = [], options = {}) {
    ...
    if (options.width && options.height) {
      ...
      this.height = Math.max(
        minHeight,
        Math.min(
          titleHeight + messageHeight + buttonsHeight,
          canvas.height * 0.8
        )
      );
